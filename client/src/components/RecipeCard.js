  ...
  return (
    ...
    >
      ...
      >
        ...
        <h2>Album:&nbsp;
          <NavLink to={`/albums/${authorData.album_id}`}>{authorData.title}</NavLink>
        </h2>
        <p>Duration: {formatDuration(recipeData.duration)}</p>
        <p>Tempo: {recipeData.tempo} bpm</p>
        <p>Key: {recipeData.key_mode}</p>
        <ButtonGroup>
